...
/**
 * InferenceAnnotatedTypeFactory is responsible for creating AnnotatedTypeMirrors that are annotated
 * with "variable" or "real" annotations. Variable annotations, represented by
 * checkers.inference.@VarAnnot, indicate an annotation with a value to be inferred. Real
 * annotations, those found in the hierarchy of the type system for which we are inferring values,
 * indicate that the given type has a constant value in the "real" type system.
 *
 * <p>Adding annotations is accomplished through three means: 1. If we have the source code for a
 * particular type, the InferenceTreeAnnotator and the VariableAnnotator will add the relevant
 * annotation (either VarAnnot or constant real annotation) based on the type's corresponding tree
 * and the rules of the InferrableChecker. If the InferrableChecker determines that a value is
 * constant then the realAnnotatedTypeFactory is consulted to get this value.
 *
 * @see checkers.inference.InferenceAnnotatedTypeFactory#annotateImplicit(com.sun.source.tree.Tree,
 *     checkers.types.AnnotatedTypeMirror)
 *     <p>2. If we do NOT have the source code then the realAnnotatedTypeFactory is used to
 *     determine a constant value to place on the given "library", i.e. from bytecode, type.
 * @see
 *     checkers.inference.InferenceAnnotatedTypeFactory#annotateImplicit(javax.lang.model.element.Element,
 *     checkers.types.AnnotatedTypeMirror)
 *     <p>3. Types representing declarations generated using methods 1 and 2 are stored via
 *     VariableAnnotator#storeElementType. If these elements are encountered again, the annotations
 *     from the stored type are copied to the annotations of the type being annotated.
 * @see
 *     checkers.inference.InferenceAnnotatedTypeFactory#annotateImplicit(javax.lang.model.element.Element,
 *     checkers.types.AnnotatedTypeMirror)
 *     <p>Note: a number of constraints are created by members of this class
 * @see checkers.inference.InferenceQualifierHierarchy
 * @see checkers.inference.InferenceTypeHierarchy
 *     <p>Finally, Variables are created while flow is being performed (every time getAnnotatedType
 *     is called on a class tree that hasn't yet been annotated). Constraints are created after flow
 *     has been for a given class when the visitor requests types.
 */
public class InferenceAnnotatedTypeFactory extends BaseAnnotatedTypeFactory {
    ...
    public static final Logger logger =
            Logger.getLogger(InferenceAnnotatedTypeFactory.class.getSimpleName());

    // Used to indicate progress in the output log.  Before calling inference, if you count the
    // number of
    ...
    protected CFAnalysis createFlowAnalysis() {
        return realChecker.createInferenceAnalysis(
                inferenceChecker, this, slotManager, constraintManager, realChecker);
    }
    ...
    //        return new InferenceTypeArgumentInference(slotManager, constraintManager,
    // variableAnnotator,
    ...
    /**
     * Copies the primary annotations on the use type "type" onto each "supertype". E.g. for a call:
     * postDirectSuperTypes( @9 ArrayList< @7 String>, List( @4 List<@7 String>, @0 Object ) ) we
     * would like supertypes to become: List( @9 List<@7 String>, @9 Object )
     *
     * <p>This does NOTHING to the type parameters of a declared type. The superTypeFinder should
     * appropriately fix these up.
     */
    @Override
    protected void postDirectSuperTypes(
            AnnotatedTypeMirror type, List<? extends AnnotatedTypeMirror> supertypes) {

        // TODO: Move postdirectSupertypes to a "copyTypeToSuperType method" that can just be called
        // by this method?
    ...
    public void postAsMemberOf(
            ...
            final AnnotatedTypeMirror owner,
    ...
    /**
     * @see
     *     org.checkerframework.checker.type.AnnotatedTypeFactory#methodFromUse(com.sun.source.tree.MethodInvocationTree)
     *     TODO: This is essentially the default implementation of
     *     AnnotatedTypeFactory.methodFromUse with a space to later TODO: add comb constraints. One
     *     difference is how the receiver is gotten. Perhaps we should just TODO: change
     *     getSelfType? But I am not sure where getSelfType is used yet
     * @param methodInvocationTree
     * @return
     */
    @Override
    public ParameterizedExecutableType methodFromUse(
            final MethodInvocationTree methodInvocationTree) {
        assert methodInvocationTree != null
                : "MethodInvocationTree in methodFromUse was null.  "
                        + "Current path:\n"
                        + getVisitorTreePath();
        ...
        ParameterizedExecutableType mType =
                substituteTypeArgs(methodInvocationTree, methodElem, methodOfReceiver);
        ...
        if (methodInvocationTree.getKind() == Tree.Kind.METHOD_INVOCATION
                && TreeUtils.isMethodInvocation(
                        methodInvocationTree, objectGetClass, processingEnv)) {
    ...
    /**
     * TODO: Similar but not the same as AnnotatedTypeFactory.constructorFromUse with space set
     * aside from TODO: comb constraints, track down the differences with constructorFromUse Note:
     * super() and this() calls
     *
     * @see
     *     org.checkerframework.checker.type.AnnotatedTypeFactory#constructorFromUse(com.sun.source.tree.NewClassTree)
     * @param newClassTree
     * @return
     */
    @Override
    public ParameterizedExecutableType constructorFromUse(final NewClassTree newClassTree) {
        assert newClassTree != null
                : "NewClassTree was null when attempting to get constructorFromUse. "
                        + "Current path:\n"
                        + getVisitorTreePath();
        ...
        if (viewpointAdapter != null) {
            viewpointAdapter.viewpointAdaptConstructor(
                    constructorReturnType, constructorElem, constructorType);
        }

        ParameterizedExecutableType substitutedPair =
                substituteTypeArgs(newClassTree, constructorElem, constructorType);
    ...
    /**
     * Get a map of the type arguments for any type variables in tree. Create a list of type
     * arguments by replacing each type parameter of exeEle by it's corresponding argument in the
     * map. Substitute the type parameters in exeType with those in the type arg map.
     *
     ...
     * @return A list of the actual type arguments for the type parameters of exeEle and exeType
     *     with it's type parameters replaced by the actual type arguments
     */
    private <EXP_TREE extends ExpressionTree> ParameterizedExecutableType substituteTypeArgs(
        ...
        if (!missingTypeVars.isEmpty()) {
            throw new BugInCF(
                    "InferenceAnnotatedTypeFactory.methodFromUse did not find a mapping for "
                            ...
                            + "in the inferred type arguments: "
                            + InferenceUtil.join(typeVarMapping));
        }
        ...
        final AnnotatedExecutableType actualExeType =
    ...
    /**
     * We override this to remove the extra isSubtype check when applying inferred annotations. (so
     * we don't get those extra isSubtype constraints).
     */
    @Override
    protected void applyInferredAnnotations(
            org.checkerframework.framework.type.AnnotatedTypeMirror type, CFValue as) {
    ...
    /**
     * TODO: Expand If we have a cached AnnotatedTypeMirror for the element then copy its
     * annotations to type else if we can get the source tree for the declaration of that element
     * visit it with the tree annotator else get the AnnotatedTypeMirror from the real
     * AnnotatedTypeFactory and copy its annotations to type
     *
    ...
    public void setRoot(final CompilationUnitTree root) {
        logger.fine(
                "\nCHANGING COMPILATION UNIT ( "
                        + compilationUnitsHandled
                        + " ): "
    ...
    public Set<AnnotationMirror> getTypeDeclarationBounds(TypeMirror type) {
        ...
        // This is to handle the special case of anonymous classes when the super class (or
        // interface)
    ...
    /**
     * Unlike the cases in type checking, in inference we should: (1) if the clause tree contains
     * explicit annotation, return the corresponding @VarAnnot (2) otherwise, return the primary
     * variable created for the clause
     *
