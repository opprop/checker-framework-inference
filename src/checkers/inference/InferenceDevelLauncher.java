 ...
 *
 * <p>Instead of using jar files of checker-framework-inference and checker-framework, {@code
 * InferenceDevelLauncher} uses the eclipse build class files to invoke {@code InferenceMain}.
 * Similar to {@code CheckerDevelMain}, this class is associated with a shell script called {@link
 * inference-dev}, which set the proper class files locations that this class would use.
 *
...
public class InferenceDevelLauncher extends InferenceLauncher {
    ...
    public static void main(String[] args) {
        ...
        assert (annotatedJDK != null)
                : ANNOTATED_JDK + " must specify a path entry to prepend to the annotated JDK";
    ...
    protected void initInferenceOptions(String[] args) {
        ...
        // overwrite distributed dirs and jars to the location that {@code
        // InferenceDevelLauncher.BINARY} indicates.
        InferenceOptions.pathToThisJar =
                null; // {@code InferenceDevelLauncher} should not be called from jar. set to null
    ...
    @Override
    /**
     * return the eclipse output directory instead of jars. the eclipse output directory is set by
     * {@code InferenceDevelLauncher.RUNTIME_BCP}
     */
    ...
    /**
     * TODO: we need to extract the utility methods in {@code CheckerMain} and {@code
     * CheckerDevelMain} out to an Util Class, change their visibility to public, then we can reuse
     * them in {@code InferenceLauncher}, {@code InferenceDevelLauncher} and {@code InferenceMain}.
     *
     * <p>This method is copied from CheckerDevelMain
     *
     * @param pathProp
     * @param pathOpts
     * @param otherPaths
