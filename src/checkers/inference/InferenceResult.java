...
public interface InferenceResult {
     ...
     *
     * @return true if inference result contains valid solutions for variable IDs. Returns false if
     *     underlying solver fails to give solution for {@link Constraint}s.
     * @see #getSolutions()
     ...
     * @return non-null solution iff {@link #hasSolution()} returns true and internal inferred
     *     result(implementation detail) contains solution for {@code varId}
     * @see #hasSolution()
     * @see #containsSolutionForVariable(int)
     */
     ...
     *
     * <p>Should be called only if {@link #hasSolution()} returns false. In this case, if an empty
     * collection is returned, it means the underlying solver doesn't support explaning unsolvable
     * reason.
     *
