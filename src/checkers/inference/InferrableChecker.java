 ...
 *
 * <p>This interface allows a checker to configure is inference behavior.
 *
...
public interface InferrableChecker {
    ...
    InferenceVisitor<?, ?> createVisitor(
            InferenceChecker checker, BaseAnnotatedTypeFactory factory, boolean infer);

    /**
     * Should inference generate variables and constraints for viewpoint adaption when accessing
     * instance members.
     */
     ...
     *
     * <p>If true, the underlying ATF will be used to look up the type of the node and an equality
     * constraint will be generated for between the VarAnnot and the annotation from the underlying
     * ATF.
     *
     * @param node the node
     ...
     *     AnnotationMirror -> ConstantSlot in order to avoid creating multiple constants for the
     *     same annotation. For parameterized qualifiers this should return false.
     */
    boolean shouldStoreConstantSlots();

    /**
     * should the inference annotations of main modifier of local variables also insert into source
     * code
     *
     * <p>Generally a checker would not want insert annotations of main modifier of local vars
     * because they can inferred by flow refinement. For some specific checkers, e.g. Ontology, they
     * may not want ignore those information.
     *
     ...
     *
     * <p>For example, in Units Checker, it is preferred to insert {@code @m}, an alias annotation,
