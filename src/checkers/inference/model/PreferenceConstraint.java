...
public class PreferenceConstraint extends Constraint {
    ...
    private PreferenceConstraint(
            VariableSlot variable, ConstantSlot goal, int weight, AnnotationLocation location) {
    ...
    protected static PreferenceConstraint create(
            VariableSlot variable, ConstantSlot goal, int weight, AnnotationLocation location) {
        if (variable == null || goal == null) {
            throw new BugInCF(
                    "Create preference constraint with null argument. Variable: "
                            + variable
                            + " Goal: "
                            + goal);
        }
