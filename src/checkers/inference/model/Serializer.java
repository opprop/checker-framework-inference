 ...
 *
 * <p>Serialization will occur for all variables and constraints either before or instead of
 * Constraint solving.
 *
 * <p>This allows us to avoid re-generating constraints for a piece of source code every time we
 * wish to solve (for instance when a new solver is written or an existing one is modified).
 *
 * <p>Type parameters S and T are used to adapt the return type of the XXXSlot visitor methods (S)
 * and the XXXConstraint visitor methods (T). Implementing classes can use the same or different
 * types for these type parameters.
 */
public interface Serializer<S, T> {
    ...
    S serialize(ComparisonVariableSlot slot);
