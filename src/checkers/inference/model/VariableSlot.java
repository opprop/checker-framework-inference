 ...
 * After the Solver is run, each VariableSlot has an assigned value which is then written to the
 * output Jaif file for later insertion into the original source code.
 *
 * <p>The {@link checkers.inference.InferenceVisitor} and other code can convert between VarAnnots
 * in AnnotatedTypeMirrors and VariableSlots, which are then used to generate constraints.
 *
 * <p>E.g. @VarAnnot(0) String s;
 *
 * <p>The above example implies that a VariableSlot with id 0 represents the possible annotations on
 * the declaration of s.
 *
 * <p>Every VariableSlot has an {@link AnnotationLocation} and the {@link RefinementVariableSlot}s
 * it is refined by.
 */
public abstract class VariableSlot extends Slot {

    /**
     * Used to locate this Slot in source code. {@code AnnotationLocation}s are written to Jaif
     * files along with the annotations determined for this slot by the Solver.
     */
