...
/**
 * Solver adapts a concrete underlying solver, e.g. Sat4j, LogiQL, Z3, etc. This class is the super
 * class for all concrete Solver sub-classes. For each concrete Solver, it adapts the type
 * constraint solving process to the underlying solver implementation.
 *
 * <p>A Solver takes type constraints from {@link checkers.inference.solver.SolverEngine}}, then
 * delegates solving constraints responsibility to the underlying solver, and transform underlying
 * solver solution as a map between an integer and an annotation mirror as the inferred result.
 *
...
public abstract class Solver<T extends FormatTranslator<?, ?, ?>> {
     ...
     * constraint-solving strategy. In general, there will be three steps in this method: 1. Calls
     * {@link #encodeAllConstraints()}, let {@link FormatTranslator} to convert constraints into the
     * corresponding encoding form. Optionally, encode well-formedness restriction if the backend
     * has it. 2. Calls the underlying solver to solve the encoding. 3. For UNSAT case, returns
     * null. Otherwise let {@link FormatTranslator} decodes the solution from the underlying solver
     * and create a map between an Integer(Slot Id) and an AnnotationMirror as it's inferred
     * annotation.
     *
     * <p>It is the concrete solver adapter's responsibility to implemented the logic of above
     * instructions and statistic collection. See {@link
    ...
    /**
     * Calls formatTranslator to convert constraints into the corresponding encoding form. See
     * {@link checkers.inference.solver.backend.maxsat.MaxSatSolver#encodeAllConstraints()}} for an
     * example.
     */
