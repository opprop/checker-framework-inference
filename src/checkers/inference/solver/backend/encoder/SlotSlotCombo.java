 ...
 * (in this case, it's the combination of {@link CombineConstraint#target} and {@link
 * CombineConstraint#decl}).
 *
 * <p>{@link BinaryConstraintEncoder} and {@link CombineConstraintEncoder} need to know the
 * combination of {@link Slot.Kind} to determine which encodeXXX() method in {@link
 * BinaryConstraintEncoder} and {@link CombineConstraintEncoder} should be called.
 *
 * <p>But the {@link Slot.Kind} that's needed here is coarser-grained than its original definition:
 ...
 * as having unknown value that is to be inferred and only the {@link
 * checkers.inference.model.VariableSlot#id} is interesting; Solver treats {@link
 * checkers.inference.model.ConstantSlot} as having a real qualifier that no inference is needed.
 *
...
public enum SlotSlotCombo {
    ...
    /**
     * A {@link java.util.Map} that caches the mapping between two booleans(in implementation,
     * converted to indecies according to the truth value) and {code SlotSlotCombo} that the two
     * booleans represent.
     */
    private static final SlotSlotCombo[][] map = new SlotSlotCombo[2][2];
