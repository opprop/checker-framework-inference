...
public class TwoQualifiersLattice extends Lattice {
    ...
    public boolean isSubtype(AnnotationMirror a1, AnnotationMirror a2) {
        if (!AnnotationUtils.containsSame(allTypes, a1)
                || !AnnotationUtils.containsSame(allTypes, a2)) {
            throw new BugInCF(
                    "Enconture invalid type when perform isSubtype judgement: "
                            + " all type qualifiers in this lattice are: "
                            + allTypes
                            + " but a1 is : + "
                            + a1
                            + " and a2 is: "
