 ...
 *
 * <p>1. Build a {@link ConstraintGraph} based on the given set of constraints. 2. Divide the
 * constraint graph to multiple sub-graphs. 3. For each sub-graphs, assign an underlying solver to
 * solve it. 4. Merge solutions of sub-graphs to get the final solution.
 *
 * <p>This solving strategy is useful when solving constraints for a type system with a huge number
 * of qualifers. Normal plain solving strategy meet exponentially increased solving time in this
 * case.
 */
public class GraphSolvingStrategy extends AbstractSolvingStrategy {
    ...
    public InferenceResult solve(
        ...
        final boolean solveInParallel =
                !"lingeling".equals(solverEnvironment.getArg(SolverEngineArg.solver))
        ...
        ConstraintGraph constraintGraph =
                generateGraph(slots, constraints, solverEnvironment.processingEnvironment);
        final long graphBuildingEnd = System.currentTimeMillis();

        // Separate constraint graph, and assign each separated sub-graph to a underlying solver to
        // solve.
        List<Solver<?>> separatedGraphSolvers =
                separateGraph(solverEnvironment, constraintGraph, slots, constraints, lattice);
        ...
        Statistics.addOrIncrementEntry(
                "graph_generation_time(ms)", (graphBuildingEnd - graphBuildingStart));
     ...
     *
     * <p>This method should be overridden in special cases, e.g, type systems with multiple type
     * hierarchies.
     *
    ...
    protected ConstraintGraph generateGraph(
            Collection<Slot> slots,
            Collection<Constraint> constraints,
     ...
     *
     * <p>Sub-class may customize their own way of separating the constraint graph by overriding
     * this method.
     *
    ...
    protected List<Solver<?>> separateGraph(
        ...
        for (Set<Constraint> independentConstraints : constraintGraph.getIndependentPath()) {
            separatedGraphSovlers.add(
                    solverFactory.createSolver(
                            solverEnvironment, slots, independentConstraints, lattice));
        }
     ...
     */
    protected List<Pair<Map<Integer, AnnotationMirror>, Collection<Constraint>>> solveInparallel(
            List<Solver<?>> underlyingSolvers) throws InterruptedException, ExecutionException {

        ExecutorService service = Executors.newFixedThreadPool(30);
        List<Future<Pair<Map<Integer, AnnotationMirror>, Collection<Constraint>>>> futures =
                new ArrayList<>();
        ...
        List<Pair<Map<Integer, AnnotationMirror>, Collection<Constraint>>> results =
                new ArrayList<>();

        for (Future<Pair<Map<Integer, AnnotationMirror>, Collection<Constraint>>> future :
                futures) {
        ...
        Statistics.addOrIncrementEntry(
                "overall_parallel_solving_time(ms)", (solvingEnd - solvingStart));
        return results;
    }
     ...
     * @return A list of Map that contains solutions from all underlying solvers.
     */
    protected List<Pair<Map<Integer, AnnotationMirror>, Collection<Constraint>>> solveInSequential(
            List<Solver<?>> underlyingSolvers) {
        ...
        Statistics.addOrIncrementEntry(
                "overall_sequential_solving_time(ms)", (solvingEnd - solvingStart));
        return results;
    }
