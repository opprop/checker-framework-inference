...
/**
 * ASTPathUtil is a collection of utilities to create ASTRecord for existing trees, as well as trees
 * that are implied to exist but are not required by the compiler (e.g. extends Object).
 */
public class ASTPathUtil {
     ...
     */
    public static ASTRecord getASTRecordForPath(
            final AnnotatedTypeFactory typeFactory, TreePath pathToNode) {
        if (pathToNode == null) {
            return null;
        }
    ...
    public static ASTRecord getConstructorRecord(ASTRecord classRecord) {
        return new ASTRecord(
                classRecord.ast, classRecord.className, AFU_CONSTRUCTOR_ID, null, ASTPath.empty());
    }

    /**
     * Some times there are trees that are implied by the source code but not explicitly written.
     * (e.g. extends Object) The AFU will create these trees if we try to insert an annotation on
     * them. Therefore, we need to create an AFU record that corresponds to the location on the
     * "implied" (non-existant) tree.
     *
     * <p>This method creates a mapping of AnnotatedTypeMirrors that are children of type and an
     * ASTRecord that corresponds to annotating the primary annotation of that type.
     *
     * @param parent The parent of the implied tree
     * @param type The type on which we are storing annotations.
     * @return A mapping of annotated type mirror to the ASTRecord that locates the primary
     *     annotation of the annotated type mirror in source code. i.e. (ATM -> locatation of
     *     primary annotation on ATM in source code)
     */
    public static IdentityHashMap<AnnotatedTypeMirror, ASTRecord> getImpliedRecordForUse(
            final ASTRecord parent, final AnnotatedTypeMirror type) {
    ...
    /**
     * A scanner that builds a mapping of ATM -> (AstRecord representing the location of the primary
     * annotation of ATM)
     */
    protected static class AFUPathMapper extends AnnotatedTypeScanner<Void, ASTRecord> {
        ...
        public ASTRecord extendParent(
                final ASTRecord parent, Tree.Kind treeKind, String type, int arg) {
            return parent.extend(treeKind, type, arg);
        }

        /**
         * @return true if this is the first time we have encountered type and therefore we stored
         *     type -> path
         */
        ...
        public Void visitIntersection(AnnotatedIntersectionType type, ASTRecord current) {
            ...
            for (AnnotatedTypeMirror bound : type.directSupertypes()) {
                ASTRecord toBound =
                        extendParent(current, Kind.INTERSECTION_TYPE, ASTPath.BOUND, boundIndex);
                visit(bound, toBound);
                boundIndex++;
            }
        ...
        public Void visitExecutable(AnnotatedExecutableType type, ASTRecord parent) {
            throw new UnsupportedOperationException(
                    "This class was not intended to create paths for methods.");
        }
        ...
        public Void visitWildcard(AnnotatedWildcardType type, ASTRecord current) {
            ...
            if (!AnnotatedTypes.isExplicitlySuperBounded(type)) {
                ...
                if (AnnotatedTypes.isExplicitlyExtendsBounded(type)) {
                    final ASTRecord toBound =
                            extendParent(current, Kind.EXTENDS_WILDCARD, ASTPath.BOUND, 0);
                    visit(type.getExtendsBound(), toBound);
                } else {
                    final ASTRecord toBound =
                            extendParent(current, Kind.UNBOUNDED_WILDCARD, ASTPath.BOUND, 0);
                    visit(type.getExtendsBound(), toBound);
                }
            } else {
                mapping.put(type.getExtendsBound(), current);
                final ASTRecord toBound =
                        extendParent(current, Kind.SUPER_WILDCARD, ASTPath.BOUND, 0);
