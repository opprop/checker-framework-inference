...
public class CopyUtil {
    ...
    public static class ClearAndCopy implements CopyMethod {
        ...
        /**
         * Clear to of any annotations and copy those from 'from' to 'to' Does not descend into
         * nested types if any
         *
    ...
    /**
     * Copy annotations from in to mod, descending into any nested types in the two
     * AnnotatedTypeMirrors. Any existing annotations will be cleared first
     *
    ...
    /**
     * Use copyAnnotations to deep copy the return type and parameter type annotations from one
     * AnnotatedExecutableType to another
     *
     * @param from The executable type with annotations to copy
     * @param to The executable type to which annotations will be copied
     */
    public static void copyParameterReceiverAndReturnTypes(
            final AnnotatedExecutableType from, AnnotatedExecutableType to) {
    ...
    private static void copyAnnotationsImpl(
        ...
        if (fromKind == PACKAGE) {
            ...
            AnnotatedDeclaredType enclosingOfFrom =
                    ((AnnotatedDeclaredType) from).getEnclosingType();
            ...
            copyAnnotationsTogether(
                    ((AnnotatedDeclaredType) from).getTypeArguments(),
            ...
            copyAnnotationsImpl(
                    fromExeType.getReturnType(), toExeType.getReturnType(), copyMethod, visited);
            // Static methods don't have a receiver.
            if (fromExeType.getReceiverType() != null) {
                copyAnnotationsImpl(
                        fromExeType.getReceiverType(),
                        toExeType.getReceiverType(),
                        copyMethod,
                        visited);
            }
            copyAnnotationsTogether(
                    fromExeType.getParameterTypes(),
                    toExeType.getParameterTypes(),
                    copyMethod,
                    visited);
            copyAnnotationsTogether(
                    fromExeType.getTypeVariables(),
                    toExeType.getTypeVariables(),
                    copyMethod,
                    visited);

        } else if (fromKind == ARRAY && toKind == ARRAY) {
            copyAnnotationsImpl(
                    ((AnnotatedArrayType) from).getComponentType(),
            ...
            copyAnnotationsImpl(
                    fromAtv.getUpperBound(), toAtv.getUpperBound(), copyMethod, visited);
            copyAnnotationsImpl(
                    fromAtv.getLowerBound(), toAtv.getLowerBound(), copyMethod, visited);
            ...
            copyAnnotationsImpl(
                    fromWct.getExtendsBound(), tpWct.getExtendsBound(), copyMethod, visited);
            copyAnnotationsImpl(
                    fromWct.getSuperBound(), tpWct.getSuperBound(), copyMethod, visited);

        } else if (fromKind.isPrimitive() && toKind.isPrimitive()) {
            // Primitives only take one annotation, which was already copied

        } else if (fromKind == NONE
                || fromKind == NULL
                || fromKind == VOID
                || toKind == NONE
                || toKind == NULL
            ...
            copyAnnotationsOnDeclaredTypeList(
                    fromAlternatives, toAlternatives, copyMethod, visited);

        } else {
            throw new BugInCF(
                    "CopyUtils.copyAnnotationsImpl: unhandled getKind results: "
                            + from
                            + " and "
                            + to
                            + "\n    of kinds: "
    ...
    /**
     * Helper method for copying annotations from a given declared type list to another declared
     * type list.
     */
    private static void copyAnnotationsOnDeclaredTypeList(
            final List<? extends AnnotatedTypeMirror> from,
        ...
        for (TypeMirror underlyingType : fromMap.keySet()) {
            if (!toMap.containsKey(underlyingType)) {
                throw new BugInCF(
                        "Unequal types found! Copy destination doesn't have this type: "
                                + underlyingType
                                + "."
                                + " from: "
                                + from
    ...
    private static void copyAnnotationsTogether(
            final List<? extends AnnotatedTypeMirror> from,
            final List<? extends AnnotatedTypeMirror> to,
            final CopyMethod copyMethod,
            final IdentityHashMap<AnnotatedTypeMirror, AnnotatedTypeMirror> visited) {
