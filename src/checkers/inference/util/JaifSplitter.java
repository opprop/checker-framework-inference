...
/**
 * Splits a JAIF file by package into many JAIFs and writes them to the outputDir. Creates a shell
 * script that will insert all of them one-by-one.
 *
 * <p>When running insert-annotation-to-source you must pass a list of Java files as arguments.
 * Sometimes this list is longer than the permissiable number of arguments on the command line (this
 * occurs for Hadoop). In these cases you can use the JaifSplitter to split the jaif into many files
 * that can be inserted individually. In order to make it easy to insert all of them, JaifSplitter
 * will also write a shell script that will insert all of the jaifs one-by-one.
 *
 * <p>Splitting jaifs also has the following effects: a) The AFU doesn't have to search as many Java
 * files when searching for the correct file to insert into. b) If the AFU crashes on a malformed
 * JAIF for one package, annotations are still inserted into the other packages c) The shell script
 * can and will report when it has completed a package and how many total packages there are.
 * Therefore, you can get some sense of progress when inserting the annotations for a large project
 *
 * <p>To use JaifSplitter from source code, create a new JaifSplitter and call jaifSplitter.split()
...
public class JaifSplitter {
     ...
     * @param outputDir the directory in which the new jaifs, created by splitting jaifFile, will be
     *     placed
     * @param commandFile the shell file in which to place the shell script for inserting all jaifs
     *     in outputDir
     * @param srcPattern in order to limit the directories searched for java file, srcPattern should
     *     be a path from the projects root to an ancestor directory of all java files e.g. for
     *     Maven project, src/main/java
     * @param annos the annotations to place in every jaif files header, these annotations should be
     *     strings of the following format: fully.qualified.AnnotationClassName[type arg, type arg]
     *     Where "type arg" specifies a property of AnnotationClassName, e.g. "int value" VarAnnot
     *     would be represented as checkers.inference.qual.VarAnnot[int value]
     */
    public JaifSplitter(
            File jaifFile,
            File outputDir,
    ...
    public static void main(String[] args) {
        ...
        new JaifSplitter(new File(args[0]), new File(args[1]), new File(args[2]), args[3], annos)
                .split();
    }

    public static void printUsage() {
        System.out.println(
                "This class splits a large jaif into a number of smaller jaifs and generates a shell script "
                        + "that will insert the jaif into source code starting at $PWD/**/srcPattern\n");
        System.out.println(
                "Usage: JaifSplitter jaifFile outputDir commandFile srcPattern anno [anno ...]");
        System.out.println("    jaifFile -- the file to split");
        System.out.println("    outputDir -- the directory in which to put the new jaifs");
        System.out.println(
                "    commandFile -- the sh file in which to place sh commands to insert the new jaifs");
        System.out.println(
    ...
    /**
     * Split a large jaif into smaller jaifs by package name. The smaller jaifs are placed in output
     * dir. This method also generates a shell script full of insert-annotations-to-source commands
     * that will insert all of the jaifs.
     *
     ...
     */
    protected static void splitAndWrite(
            File jaifFile, File outputDir, File commandFile, String srcPattern, String header) {
        ...
        for (JaifPackage jaifPackage : reader) {
            ...
            if (!visitedFiles.contains(outputJaif)) {
                ...
                insertionCommands.add(
                        makeInsertionCommand(outputJaif, jaifPackage.name, srcPattern));
        ...
        System.out.println(
                "Writing "
                        + insertionCommands.size()
                        + " insertions commands to file:\n"
                        + commandFile.getAbsolutePath());
        writeInsertShellScript(commandFile, insertionCommands, visitedFiles);
    }

    public static void writeInsertShellScript(
            File commandFile, Set<String> insertionCommands, Set<File> jaifs) {

        List<String> shFileCommands = new ArrayList<>(insertionCommands.size() + 6);
        shFileCommands.add("if [ -z \"$1\" ]");
        shFileCommands.add("then");
        shFileCommands.add(
                "echo \"You must specify 1 argument, the root of the source directory that contains "
        ...
        while (cmdsIter.hasNext()) {
            ...
            shFileCommands.add(
                    "echo \"running insertion ("
                            + current
                            + " / "
                            + total
                            + ") on file "
                            + jaif.getAbsolutePath()
                            + "\"");
    ...
    /**
     * Takes a list of fully qualified annotations and turns them into Jaif headers that allow them
     * to be used within insertions
     *
    ...
    /**
     * Given a jaif file and the corresponding package name for that jaif file, return an
     * insert-annotations-to-source command that will find the correct files and insert the jaif
     */
    private static String makeInsertionCommand(
            File jaifFile, String packageName, String srcPattern) {
        String packagePath = packageName.replace(".", File.separator);

        // right now this is tailored for hadoop
        String srcPath =
                "**/" + srcPattern + File.separator + packagePath + File.separator + "*.java";
        String findPackageSourcesCmd = " `find $1 -path " + srcPath + "`";

        return "insert-annotations-to-source -i \""
                + jaifFile.getAbsolutePath()
                + "\" "
                + findPackageSourcesCmd;
    }
}
