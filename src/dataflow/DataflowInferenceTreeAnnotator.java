...
public class DataflowInferenceTreeAnnotator extends InferenceTreeAnnotator {
    ...
    public Void visitLiteral(final LiteralTree literalTree, final AnnotatedTypeMirror atm) {
        if (!literalTree.getKind().equals(Kind.NULL_LITERAL)) {
            AnnotationMirror anno =
                    DataflowUtils.generateDataflowAnnoFromLiteral(
                            literalTree, this.realTypeFactory.getProcessingEnv());
            replaceATM(atm, anno);
        } else {
    ...
    public Void visitNewClass(final NewClassTree newClassTree, final AnnotatedTypeMirror atm) {
        ...
        ((DataflowAnnotatedTypeFactory) this.realTypeFactory)
                .getTypeNameMap()
                .put(tm.toString(), tm);
        AnnotationMirror anno =
                DataflowUtils.genereateDataflowAnnoFromNewClass(
                        atm, this.realTypeFactory.getProcessingEnv());
        replaceATM(atm, anno);
    ...
    public Void visitMethodInvocation(
            MethodInvocationTree methodInvocationTree, final AnnotatedTypeMirror atm) {
    ...
    private void replaceArrayComponentATM(AnnotatedArrayType arrayAtm) {
        ...
        AnnotationMirror componentAnno =
                DataflowUtils.genereateDataflowAnnoFromByteCode(
                        componentAtm, this.realTypeFactory.getProcessingEnv());
