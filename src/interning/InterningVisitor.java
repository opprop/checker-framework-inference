...
public final class InterningVisitor
        extends InferenceVisitor<InterningChecker, BaseAnnotatedTypeFactory> {
    ...
    public InterningVisitor(
            InterningChecker checker,
    ...
    public Void visitBinary(BinaryTree node, Void p) {
        ...
        if (leftOp.getKind() == Tree.Kind.NULL_LITERAL
                || rightOp.getKind() == Tree.Kind.NULL_LITERAL) return super.visitBinary(node, p);
        ...
        if (id instanceof JCTree.JCAnnotatedType) {
            ...
            for (JCAnnotation jcann : jcat.getAnnotations()) {
                ...
                if (realChecker.INTERNED.getAnnotationType().equals(cl)) {
                    // Forbid "new @Interned myClass()". You can add a @SuppressWarnings for this.
                    // We could add an
                    // option that would relax this rule and not forbid it if it's common enough.
                    // The point is to issue
                    // an error if we see thigns like "new @Interned String("foo")" which is
                    // obviously not interned.
                    // Don't enforce this rule, however, if the class is annotated with @Interned or
                    // the constructor.
     ...
     *
     * <p>Returns true even if a method does not override {@link Object.equals}, because of the
     * common idiom of writing an equals method with a non-Object parameter, in addition to the
     * equals method that overrides {@link Object.equals}.
     *
    ...
    /**
     * Given <code>a == b</code>, where a has type A and b has type B, don't issue a warning when
     * either the declaration of A or that of B is annotated with @Interned because <code>a == b
     * </code> will be true only if a's run-time type is B (or lower), in which case a is actually
     * interned.
     *
     * <p>
     */
    ...
    private Element getThis(Scope scope) {
        for (Element e : scope.getLocalElements())
            if (e.getSimpleName().contentEquals("this")) return e;
        return null;
    }
